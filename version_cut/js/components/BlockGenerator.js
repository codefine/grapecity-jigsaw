...
BlockGenerator.prototype.swapBlockState = function(id1, id2) {
    var index1 = this.state.findIndex(function(block) {
        return block.id === id1;
    });
    var index2 = this.state.findIndex(function(block) {
        return block.id === id2;
    });
...
BlockGenerator.prototype.disruptBlocks = function() {
    ...
    this.state.forEach(function(block, i) {
        ...
        THIS.updateBlockState(block.id, {
            l: position[0],
            t: position[1],
            r: rotation
        });
...
BlockGenerator.prototype.swapBlocks = function(draggedState, replacedState) {
    ...
    this.updateBlockState(draggedState.id, {
        l: targetPosition[0],
        t: targetPosition[1]
    });
    this.updateBlockState(replacedState.id, {
        l: originPosition[0],
        t: originPosition[1]
    });
};
